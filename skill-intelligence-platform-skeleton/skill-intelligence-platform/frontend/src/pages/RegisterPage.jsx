...
export default function RegisterPage() {
  ...
  const next = async () => {
    ...
    if (step === 3) {
      ...
      // If quiz not submitted yet, submit and reveal results view on Step 3
      if (!quizSubmitted) {
        setQuizSubmitted(true);
        return;
      }

      // If already submitted and user clicks Continue, advance to Step 4
  ...
  const back = () => {
    setError('');
    if (step === 3 && quizSubmitted) {
      // If on step 3 viewing quiz results, clicking Back lets user re-examine or edit quiz answers
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            {generatingQuiz ? (
              ...
              /* ── Mode A: Answering the Quiz (No answers/score shown at bottom) ── */
              <>
                <InfoBox color={dynamicQuestions ? "#f0fdf4" : "#eff6ff"} border={dynamicQuestions ? "#bbf7d0" : "#bfdbfe"} text={dynamicQuestions ? "#166534" : "#1e40af"}>
                  🤖 <strong>{dynamicQuestions ? "AI-Personalized Quiz" : "AI Diagnostic Quiz"}.</strong> {dynamicQuestions
                    ? `These 5 diagnostic questions were dynamically generated specifically for your role (${s1.role}), department (${s1.department}), designation (${s1.designation || 'Officer'}), and extracted CV profile.`
                    : `These questions test core baseline concepts for ${s1.role || 'Officer'}.`} Answer all questions to complete the assessment.
                </InfoBox>
                  ...
                  {questions.map((q, i) => (
                    <div key={i} style={{ background: '#f9fafb', borderRadius: '12px', padding: '1.1rem 1.25rem', border: '1px solid #e5e7eb' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        {q.opts.map((opt, j) => (
                          <label key={j} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', cursor: 'pointer', padding: '0.45rem 0.75rem', borderRadius: '8px', background: quizAnswers[i] == j ? '#eef2fb' : 'white', border: `1.5px solid ${quizAnswers[i] == j ? NAVY : '#e5e7eb'}`, transition: 'all 0.15s' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc', padding: '0.75rem 1rem', borderRadius: '10px', border: '1px solid #e2e8f0', marginTop: '0.5rem' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>
                    Progress: {Object.keys(quizAnswers).length} of {questions.length} questions answered
                  </span>
                  {Object.keys(quizAnswers).length === questions.length ? (
                    <span style={{ fontSize: '0.78rem', color: GREEN, fontWeight: 700 }}>
                      ✓ All questions answered! Click "Submit Quiz & View Results" below.
                    </span>
                  ) : (
                    <span style={{ fontSize: '0.78rem', color: ORANGE, fontWeight: 600 }}>
              ...
              /* ── Mode B: Quiz Completed — Results & Detailed Answer Review ── */
              <>
                {(() => {
                  let correct = 0;
                  questions.forEach((q, i) => { if (parseInt(quizAnswers[i]) === q.ans) correct++; });
                  ...
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      {/* Score Header Card */}
                      <div style={{ background: isGood ? '#f0fdf4' : '#fffbeb', border: `2px solid ${isGood ? '#86efac' : '#fde68a'}`, borderRadius: '16px', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem', boxShadow: '0 4px 14px rgba(0,0,0,0.05)' }}>
                        <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isGood ? GREEN : '#d97706', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
                          <span style={{ color: 'white', fontWeight: 900, fontSize: '1.2rem', fontFamily: "'Poppins', sans-serif" }}>{pct}%</span>
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 800, color: NAVY, fontSize: '1.05rem', marginBottom: '0.2rem' }}>
                            Diagnostic Assessment Result: {correct} / {questions.length} Correct ({pct}%)
                          </div>
                          <div style={{ fontSize: '0.82rem', color: '#4b5563', lineHeight: 1.5 }}>
                            🎯 Excellent! Your diagnostic score will <strong>automatically pre-fill your competency baseline sliders</strong> in Step 4. Review your detailed answer breakdown below.
                          </div>
                        </div>
                      </div>

                      {/* Detailed Question Review */}
                      <div style={{ fontWeight: 700, color: NAVY, fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        📋 Detailed Question & Answer Review
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {questions.map((q, i) => {
                          const userAns = parseInt(quizAnswers[i]);
                          const isCorrect = userAns === q.ans;
                          return (
                            <div key={i} style={{ background: isCorrect ? '#f0fdf4' : '#fef2f2', borderRadius: '12px', padding: '1.1rem 1.25rem', border: `1.5px solid ${isCorrect ? '#bbf7d0' : '#fecaca'}` }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem', gap: '0.5rem' }}>
                                <div style={{ fontWeight: 700, color: NAVY, fontSize: '0.875rem' }}>
                                  Q{i + 1}. {q.q}
                                </div>
                                <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '6px', color: isCorrect ? GREEN : '#dc2626', background: isCorrect ? '#dcfce7' : '#fee2e2', flexShrink: 0 }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                                {q.opts.map((opt, j) => {
                                  ...
                                  return (
                                    <div key={j} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.45rem 0.75rem', borderRadius: '8px', background: bg, border: `1.5px solid ${border}`, fontSize: '0.82rem', color: textCol, fontWeight: (isRightAns || isSelected) ? 600 : 400 }}>
                                      <span>{opt}</span>
                                      {isRightAns && <span style={{ fontSize: '0.72rem', fontWeight: 700, color: GREEN }}>Correct Answer ✓</span>}
                                      {isSelected && !isRightAns && <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#dc2626' }}>Your Choice ✗</span>}
                                    </div>
                                  );
                                })}
                              </div>
