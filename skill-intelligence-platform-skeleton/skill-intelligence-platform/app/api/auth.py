...
def switch_demo_persona(payload: DemoSwitchRequest, db: Session = Depends(get_db)):
    """
    SIH Demo Persona Switcher:
    Instantly returns a valid JWT token for a pre-configured demo officer persona (SSO, JSO, MCTP-II, MCTP-III, or Admin).
    """
    role_code = payload.role_code.upper()
    role = db.query(Role).filter_by(code=role_code).first()

    # Find an official matching this role
    if role_code == "ADMIN":
        official = db.query(Official).filter_by(is_admin=True).first()
    else:
        official = db.query(Official).filter_by(role_id=role.id if role else None).first() if role else None

    if not official:
        official = db.query(Official).first()

    if not official:
        raise HTTPException(status_code=404, detail="No demo official available.")

    token = create_access_token(data={
        "sub": official.id,
        "email": official.email,
        "role": role_code if role_code != "ADMIN" else (official.role.code if official.role else "SSO"),
        "is_admin": official.is_admin or (role_code == "ADMIN"),
    })
